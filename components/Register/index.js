...
import ekyccooporate from '@components/Upload/state';
...
const Register = () => {
    ...
    const setItem = useSetRecoilState(ekyccooporate);
    ...
    useEffect(() => {
        if (gpdkkd?.company_name != null && bonhiem?.company_name != null) {
            if (gpdkkd.company_name.replace(/ /g, '') == bonhiem.company_name.replace(/ /g, '')) {
                setMatch1(true);
            } else {
                _openModal3();
                setMatch1(false);
        ...
        if (gpdkkd.boss_name != null && gttt.name != null) {
            if (gpdkkd.boss_name.replace(/ /g, '') == gttt.name.replace(/ /g, '')) {
                setMatch2(true);
            } else {
                _openModal3();
                setMatch2(false);
        ...
        if (bonhiem.web?.name && gtttktt.name != null) {
            console.log(bonhiem.web?.name);
            console.log(gtttktt.name);
            if (bonhiem.web?.name.replace(/ /g, '') == gtttktt.name.replace(/ /g, '')) {
                setMatch3(true);
            } else {
                _openModal3();
                setMatch3(false);
    ...
    useEffect(() => {}, [match_1, match_2, match_3]);
    ...
    const nextPage = (e) => {
        router.push({ pathname: '/register/compare', data: { gpdkkd: gpdkkd, gttt: gttt, bonhiem: bonhiem, gtttktt: gtttktt } });
    };
    ...
    return (
        <div className={`p-6`}>
            <div className={``}>
                ...
                <div className="container mx-auto py-6">
                    ...
                    <div className={`p-9 flex justify-between`}>
                        ...
                        <button className={`py-2 px-9 bg-yellow-400 text-black rounded w-52`} onClick={nextPage}>
                            Đối chiếu hồ sơ
                        </button>
                    </div>
                </div>
            ...
            <br />
            {/* Modals 2 */}
            <a href="#" className={`text-link underline`} onClick={_openModal2}>
                Lỗi hình ảnh
            </a>
            <Modal open={openModal2} onClose={_closeModal2} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                <Box className={`px-16 py-9 custom-modal`}>
                    <ModalImageError />
                </Box>
            </Modal>
            <br />
            {/* Modals 3 */}
            <a href="#" className={`text-link underline`} onClick={_openModal3}>
                Lỗi không khớp thông tin
            </a>
            <Modal open={openModal3} onClose={_closeModal3} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                <Box className={`px-16 py-9 custom-modal`}>
                    <ModalInfoError />
                </Box>
            </Modal>
            <br />
            {/* Modals 4 */}
            <a href="#" className={`text-link underline`} onClick={_openModal4}>
                Lỗi định danh khách hàng thất bại
            </a>
            <Modal open={openModal4} onClose={_closeModal4} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                <Box className={`px-16 py-9 custom-modal`}>
                    <ModalIdentityError />
                </Box>
            </Modal>
