...
const CompleteFinal = (page) => {
    ...
    return (
        <>
            ...
            <div className={`flex justify-center mb-3 text-green-500`}><AiFillCheckCircle size={36} /></div>
            <h1 className={`font-bold text-center text-xl text-black mb-2`}>Đăng kí tài khoản thành công</h1>
            <p className={`text-black text-center mb-6`}>PVcomBank sẽ thực hiện xác minh hồ sơ thông tin của Doanh nghiệp theo quy định<br />của NHNN trong vòng 01 ngày làm việc và sẽ phản hồi kết quả thông qua Email/SMS</p>
                ...
                <div className="accordion max-w-screen-md w-full border border-gray-200 -mt-px" id="accordionExample">
                    <div className="accordion-item py-3 px-9 border-b border-gray-200">
                        <h2 className="accordion-header mb-0 relative" id="headingOne">
                            <button className="relative flex justify-between items-center w-full transition focus:outline-none" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded={tab == 'step1' ? 'true' : 'false'} aria-controls="collapseOne">
                                <div className={`flex flex-col items-start`}>
                                    <span className={`font-bold text-normal text-sky-500 flex items-center`}>Dịch vụ Internet Banking KHDN 
                                    {tab == `step1` ? (
                                        <AiOutlineDownCircle className={`ml-3`} size={`18`} />
                                    ) : (
                                        <AiOutlineRightCircle className={`ml-3`} size={`18`} />
                                    )}
                                    </span>
                                    <p className={`text-black`}>12.000đ/tháng</p>
                                </div>
                                <a href="#" className={`w-36 py-2 px-4 bg-yellow-100 border border-yellow-400 rounded`}>Đăng ký ngay</a>
                            </button>
                        </h2>
                        <div id="collapseOne" className={`accordion-collapse collapse pb-9 ` + (tab == `step1` ? 'show' : '')} aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            ...
                            <div>
                                Internet Banking là dịch vụ Ngân hàng điện tử của PVcomBank nhằm mang đến cho Khách hàng một phương thức quản lý tài chính an toàn và tiện lợi.<br />
                    ...
                    <div className="accordion-item py-3 px-9">
                        <h2 className="accordion-header mb-0 relative" id="heading2">
                            <button className="relative flex justify-between items-center w-full transition focus:outline-none" type="button" data-bs-toggle="collapse" data-bs-target="#collapse2" aria-expanded={tab == 'step2' ? 'true' : 'false'} aria-controls="collapse2">
                                <div className={`flex flex-col items-start`}>
                                    <span className={`font-bold text-normal text-sky-500 flex items-center`}>Dịch vụ SMS Banking KHDN
                                    {tab == `step2` ? (
                                        <AiOutlineDownCircle className={`ml-3`} size={`18`} />
                                    ) : (
                                        <AiOutlineRightCircle className={`ml-3`} size={`18`} />
                        ...
                        <div id="collapse2" className={`accordion-collapse collapse pb-9 ` + (tab == `step2` ? 'show' : '')} aria-labelledby="heading2" data-bs-parent="#accordionExample">
                            <div className={`border-t border-dashed border-sky-100 mt-6 mb-6`}></div>
                            <div>
                                SMS Banking PVcombank là một dịch vụ ngân hàng điện tử được PVcombank triển khia nhằm thông báo đến số điện thoại đăng ký số dư tài khoản hiện tại khi có sự biến động như có người chuyển tiền, rút tiền, chuyển khoản cho người khác.<br />

                                Bên cạnh đó, khi sử dụng SMS Banking PVcombank thì bạn có hưởng nhiều lợi ích như:<br />
                                - Truy vấn các giao dịch gần nhất, lãi suất, tỷ giá ... nhanh chóng, không cần phải ra chi nhánh/phòng giao dịch PVcombank.<br />
                                - Kiểm soát được số dư tài khoản của mình, từ đó chi tiêu hợp lý hơn.<br />
                                - Kiểm tra số tài khoản PVcombank dễ dàng.<br />
                            </div>
                        </div>
                    </div>
                    {/* Tab 2 */}
