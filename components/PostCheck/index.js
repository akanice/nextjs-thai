...
function createData(ttc,cif,phone,profile_ekyc,log,pc_info,pc_user,pc_regservice) {
    ...
    let _status;
    {data_pc_info.status == "approved" ?  _status = `bg-white`
      : data_pc_info.status == "pending" ? _status = `bg-cyan-50` 
      : _status = `bg-red-50`}
    ...
    return (
        <div>
            {data.status == `approved` ? (
                ...
                <>
                    <div className={`text-lg font-bold uppercase `+(data.status == `pending` ? 'text-green-400' : 'text-red-400')}>
                        {data.status == `pending` ? 'Chờ duyệt' : 'Từ chối'}
                    </div>
                    {(data.status == `reject`) ? (
            ...
            <Paper sx={{ width: '100%'}} className={`mt-3`}>
                <TableContainer sx={{ width: "100%" }}>
                    ...
                    <TableBody>
                        ...
                        .map((row) => {
                            return (
                                <tr className={`text-center shadow my-2 `+ row._status}>
